...
module hsync(input clk, output reg hsync_out, output reg blank_out, output reg newline_out, output [10:0] pixel_count);
...
module vsync(input newline_in, output reg vsync_out, output reg blank_out, output [10:0] pixel_count);
...
module data(input clk, input done, input hblank, input vblank, input [10:0] horizontal_count, input [10:0] vertical_count, 
	...
	always @ (posedge clk) begin
		if (hblank == 1 || vblank == 1) begin
			r_reg <= 4'b0; g_reg <= 4'b0; b_reg <= 4'b0;
		end
		//Column 0  x=0..39
		else if (horizontal_count < 40) begin
			r_reg <= (vertical_count > scaled_f8)  ? 4'b1100 : 4'b0; g_reg <= 4'b0; b_reg <= 4'b0;
		end
		//Column 1  x=40..79
		else if (horizontal_count >= 40 && horizontal_count < 80) begin
			r_reg <= (vertical_count > scaled_f9)  ? 4'b1100 : 4'b0; g_reg <= 4'b0; b_reg <= 4'b0;
		end
		//Column 2  x=80..119
		else if (horizontal_count >= 80 && horizontal_count < 120) begin
			r_reg <= (vertical_count > scaled_f10) ? 4'b1100 : 4'b0; g_reg <= 4'b0; b_reg <= 4'b0;
		end
		//Column 3  x=120..159
		else if (horizontal_count >= 120 && horizontal_count < 160) begin
			r_reg <= (vertical_count > scaled_f11) ? 4'b1100 : 4'b0; g_reg <= 4'b0; b_reg <= 4'b0;
		end
		//Column 4  x=160..199
		else if (horizontal_count >= 160 && horizontal_count < 200) begin
			r_reg <= (vertical_count > scaled_f12) ? 4'b1100 : 4'b0; g_reg <= 4'b0; b_reg <= 4'b0;
		end
		//Column 5  x=200..239
		else if (horizontal_count >= 200 && horizontal_count < 240) begin
			r_reg <= (vertical_count > scaled_f13) ? 4'b1100 : 4'b0; g_reg <= 4'b0; b_reg <= 4'b0;
		end
		//Column 6  x=240..279
		else if (horizontal_count >= 240 && horizontal_count < 280) begin
			r_reg <= (vertical_count > scaled_f14) ? 4'b1100 : 4'b0; g_reg <= 4'b0; b_reg <= 4'b0;
		end
		//Column 7  x=280..319
		else if (horizontal_count >= 280 && horizontal_count < 320) begin
			r_reg <= (vertical_count > scaled_f15) ? 4'b1100 : 4'b0; g_reg <= 4'b0; b_reg <= 4'b0;
		end
		//Column 8  x=320..359  -- FIXED: was f0 (DC bin, always pegged), now f1
		else if (horizontal_count >= 320 && horizontal_count < 360) begin
			r_reg <= (vertical_count > scaled_f1)  ? 4'b1100 : 4'b0; g_reg <= 4'b0; b_reg <= 4'b0;
		end
		//Column 9  x=360..399
		else if (horizontal_count >= 360 && horizontal_count < 400) begin
			r_reg <= (vertical_count > scaled_f2)  ? 4'b1100 : 4'b0; g_reg <= 4'b0; b_reg <= 4'b0;
		end
		//Column 10  x=400..439
		else if (horizontal_count >= 400 && horizontal_count < 440) begin
			r_reg <= (vertical_count > scaled_f3)  ? 4'b1100 : 4'b0; g_reg <= 4'b0; b_reg <= 4'b0;
		end
		//Column 11  x=440..479
		else if (horizontal_count >= 440 && horizontal_count < 480) begin
			r_reg <= (vertical_count > scaled_f4)  ? 4'b1100 : 4'b0; g_reg <= 4'b0; b_reg <= 4'b0;
		end
		//Column 12  x=480..519
		else if (horizontal_count >= 480 && horizontal_count < 520) begin
			r_reg <= (vertical_count > scaled_f5)  ? 4'b1100 : 4'b0; g_reg <= 4'b0; b_reg <= 4'b0;
		end
		//Column 13  x=520..559
		else if (horizontal_count >= 520 && horizontal_count < 560) begin
			r_reg <= (vertical_count > scaled_f6)  ? 4'b1100 : 4'b0; g_reg <= 4'b0; b_reg <= 4'b0;
		end
		//Column 14  x=560..599
		else if (horizontal_count >= 560 && horizontal_count < 600) begin
			r_reg <= (vertical_count > scaled_f7)  ? 4'b1100 : 4'b0; g_reg <= 4'b0; b_reg <= 4'b0;
		end
		//Column 15  x=600..639  -- f0 (DC bin) tucked at far right
		else if (horizontal_count >= 600 && horizontal_count < 640) begin
			r_reg <= (vertical_count > scaled_f0)  ? 4'b1100 : 4'b0; g_reg <= 4'b0; b_reg <= 4'b0;
		end
		else begin
			r_reg <= 4'b0; g_reg <= 4'b0; b_reg <= 4'b0;
		end
...
module VGA_generator(input clk, input done, output vsync, output hsync, output [3:0] r, output [3:0] g, output [3:0] b,
	...
	always @ (posedge clk) begin
		if (done) begin
			f0_reg  <= f0;   f1_reg  <= f1;   f2_reg  <= f2;   f3_reg  <= f3;
			f4_reg  <= f4;   f5_reg  <= f5;   f6_reg  <= f6;   f7_reg  <= f7;
			f8_reg  <= f8;   f9_reg  <= f9;   f10_reg <= f10;  f11_reg <= f11;
			f12_reg <= f12;  f13_reg <= f13;  f14_reg <= f14;  f15_reg <= f15;
		end
	end
